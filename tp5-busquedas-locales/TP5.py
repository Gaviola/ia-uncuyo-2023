...
class Board:
    ...
    def generate_queens(self, fixed_column=False):
        ...
        # Genera reinas aleatoriamente en cualquier posicion
            ...
            for i in range(self.size):
                queen = (random.randint(0, self.size - 1), random.randint(0, self.size - 1))
                self.queens.append(queen)
...
def hill_climbing(board, max_states=1000):
    ...
    while not local_max_found:
        ...
        best_neighbor, best_neighbor_cost, states_number = find_best_neighbors(best_solution)
        states_used += states_number

        # Si el costo del mejor vecino es igual o mayor que el costo actual, detenerse
        if (best_neighbor_cost >= best_solution_cost or states_used >= max_states) or best_neighbor_cost == 0:
...
def anneling(board, max_states=1000, initial_value=1000, decrease_rate=0.9):
    ...
    for states_visited in range(max_states + 1):
        if current_cost == 0:
            ...
            return current_solution, current_cost, final_states  # Encontramos una solución válida
        ...
        if delta_cost < 0 or random.random() < acceptance_probability(delta_cost, current_value):
...
def mutate(individual, board_size, mutation_rate=0.5):
    ...
    if random.random() < mutation_rate:
        ...
        individual.queens[index_to_mutate] = (new_column, individual.queens[index_to_mutate][1])
    if random.random() < 1 - mutation_rate:
        index_to_mutate = random.randint(0, board_size - 1)
        new_row = random.randint(0, board_size - 1)
        individual.queens[index_to_mutate] = (individual.queens[index_to_mutate][0], new_row)
...
def genetic_algorithm(board_size, population_size=50, max_generations=1000, mutation_rate=0.5):
...
def execute(attempts, size):
    ...
    with open(
        ...
        writer.writerow(["Algorithm_name", "Cost", "States", "Solution_found", "Time"])
        for i in range(attempts):
            board_hill = Board(size)
            ...
            if cost_hill == 0:
                solution_found = True
            writer.writerow(["Hill Climbing", cost_hill, states_hill, solution_found, end_time - start_time])

            # Simulated Annealing
            solution_found = False
            start_time = time.time()
            solution_annealing, cost_annealing, states_anneling = anneling(board_annealing)
            end_time = time.time()
            ...
            if cost_annealing == 0:
                solution_found = True
            writer.writerow(["Simulated Annealing", cost_annealing, states_anneling, solution_found, end_time - start_time])

            # Genetic Algorithm
            solution_found = False
            start_time = time.time()
            solution_genetic, cost_genetic, states_genetic = genetic_algorithm(size)
            end_time = time.time()
            ...
            if cost_genetic == 0:
                solution_found = True
            writer.writerow(["Genetic Algorithm", cost_genetic, states_genetic, solution_found, end_time - start_time])
...
def print_plotbox(result: dict):
    ...
    plt.title("Algoritmos de Búsqueda Local")
    plt.xlabel("Algoritmo de Búsqueda Local")
    ...
    print(f"Tiempo de Ejecucion de Hill Climbing: {statistics.mean(result['Hill Climbing'])}")
    print(f"Tiempo de Ejecucion de Simulated Annealing: {statistics.mean(result['Simulated Annealing'])}")
    print(f"Tiempo de Ejecucion de Genetic Algorithm: {statistics.mean(result['Genetic Algorithm'])}")
